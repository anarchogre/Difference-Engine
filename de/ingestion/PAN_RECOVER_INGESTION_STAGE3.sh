...
OUT="$TREE_HOME/PAN_INGESTION_RECOVERY_$TS-STAGE3"
...
echo "=== PAN — INGESTION RECOVERY STAGE 3 ==="
...
# 1. Preserve exact harness identity and compare with any retired copies.
{
...
# 2. Capture imports / execution contract without dumping it to terminal.
{
...
# 3. Capture nearby service/test tree.
find "$CURRENT/workspace/operational/ingestion/service" \
  -maxdepth 4 -type f -print 2>/dev/null | sort \
  > "$OUT/03_SERVICE_TREE.txt"

# 4. Run ONLY the non-retired primary recovered harness.
#    Give it both repository root and service root on PYTHONPATH.
set +e
(
  cd "$CURRENT"
...
# 5. Preserve post-run Git state so any unexpected mutation is visible.
{
