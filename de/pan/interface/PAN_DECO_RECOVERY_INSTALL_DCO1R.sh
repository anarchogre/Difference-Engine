...
def discover_chat():
    ...
    for app in desktop_apps():
        ...
        if best >= 0:
            app_candidates.append((best, app))

    if not composer_candidates:
        # Fallback: any entry whose subtree includes send/copy UI.
        for app in desktop_apps():
            has_send = False
            entries = []
            for obj in walk(app):
                r = role_name(obj)
                n = obj_name(obj).lower()
                if r in {"button", "push button"} and "send" in n:
                    has_send = True
                if r == "entry":
                    entries.append(obj)
            if has_send:
                for e in entries:
                    composer_candidates.append((1, app, e))

    if not composer_candidates:
        raise DecoError("COMPOSER_DISCOVERY=FAIL")

    # Preference order: ChatGPT desktop-style app first, Firefox second.
    ...
    composer_candidates.sort(key=app_priority, reverse=True)
...
def copy_code_buttons(app):
    ...
    for obj in walk(app):
        ...
        if (
            "copy code" in n
            or n in {"copy code to clipboard", "copy code"}
        ):
            out.append(obj)
    return out
...
def clipboard_set(data: bytes):
    ...
    if SESSION == "x11":
        ...
        clip.store()
        # Round-trip verification while this process owns the selection.
