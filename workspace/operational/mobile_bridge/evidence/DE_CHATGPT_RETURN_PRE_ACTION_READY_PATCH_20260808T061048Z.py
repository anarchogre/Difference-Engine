...
def payload_present(composer, payload):
    # Preserve the already-validated single-node path.
    stack = [composer]
    ...
    # Multiline plain text is exposed by Firefox/ChatGPT
    # as one direct composer child per clipboard line.
    #
    # Evidence:
    # composer itself contains one U+FFFC per child;
    # direct child text joined with literal LF reproduces
    # the original clipboard payload.
    try:
        parts = []

        for j in range(composer.get_child_count()):
            child = composer.get_child_at_index(j)
