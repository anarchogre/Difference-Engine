...
OUT="$TREE_HOME/PAN_JSON_PRESERVATION_IMPLEMENT_$TS-STAGE26B"
...
echo "=== PAN — JSON PRESERVATION IMPLEMENTATION STAGE 26B ==="
...
# Semantic preflight: inspect the parsed_data dict itself instead of
# comparing ast.unparse() quote style.
parsed_assign = None
for stmt in target.body:
    if not isinstance(stmt, ast.Assign):
        continue
    if not any(isinstance(t, ast.Name) and t.id == "parsed_data" for t in stmt.targets):
        continue
    if isinstance(stmt.value, ast.Dict):
        parsed_assign = stmt
        break

if parsed_assign is None:
    raise SystemExit("BLOCKER: parsed_data dict assignment missing")

keys = []
for k in parsed_assign.value.keys:
    if isinstance(k, ast.Constant) and isinstance(k.value, str):
        keys.append(k.value)

if set(keys) != {"kind", "turns", "commands"}:
    raise SystemExit(
        "BLOCKER: output normalizer drift; parsed_data keys="
        + repr(sorted(keys))
    )

# Confirm those values are still derived from parsed.get(...).
seen_get_keys = set()
for node in ast.walk(parsed_assign.value):
    ...
    if not (
        ...
        and node.func.value.id == "parsed"
        and node.args
        and isinstance(node.args[0], ast.Constant)
        and isinstance(node.args[0].value, str)
    ):
        continue
    seen_get_keys.add(node.args[0].value)

required_gets = {"kind", "turns", "commands"}
if not required_gets.issubset(seen_get_keys):
    raise SystemExit(
        "BLOCKER: output normalizer drift; parsed.get keys="
        + repr(sorted(seen_get_keys))
    )

# No JSON-preservation branch should already exist.
json_branch_present = False
for node in ast.walk(target):
    ...
    if "parsed.get('kind')=='json'" in compact:
        json_branch_present = True
        break

if json_branch_present:
    raise SystemExit(
        "BLOCKER: JSON document preservation already present unexpectedly"
    )

print(f"NORMALIZER_LINE={target.lineno}")
print("NORMALIZER_KEYS=" + repr(sorted(keys)))
print("PARSED_GET_KEYS=" + repr(sorted(seen_get_keys)))
...
PAN_JSON_PRESERVATION_IMPLEMENT_STAGE26B
UTC=$TS
STATUS=FAIL
FAIL_REASON=$reason
ROLLBACK=COMPLETE
...
fn = parser_for(Path("/tmp/PAN_STAGE26B.json"))
